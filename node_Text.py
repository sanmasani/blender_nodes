...
class CommonVariables(bpy.types.Object):
    node_tree_name = ""
    node_id_name = ""
    info_text_01 = "Select this Text Input node with click!"
    info_text_02 = "Click into socket for refresh!"
...
class SvTextInOp(bpy.types.Operator):
    ...
    def execute(self, context):        
        ...
        if node_tree.nodes[ CommonVariables.node_id_name ].select == False:
            self.report({'INFO'}, CommonVariables.info_text_01 )
            node_tree.nodes[ CommonVariables.node_id_name ].select = True
        else:
            node = bpy.data.node_groups[self.name_tree].nodes[self.name_obj]
            node.load()
            self.report({'INFO'}, CommonVariables.info_text_02 )
        return {'FINISHED'}
...
# choose which socket to interpretate data as
    ...
    #    layout.prop(self,"file","File") external file, TODO
        ...
        CommonVariables.node_id_name = self.name        
...
#         # store data
        ...
        space = bpy.context.space_data
        node_tree = space.node_tree
        node_active = bpy.context.active_node        
        links = node_tree.links
        
        # store links to array
        socketLinksArray = []        
        for socketLinks in links:
            if node_active.name == socketLinks.from_node.name:
                socketLinksArray.append([ socketLinks.from_node.name, socketLinks.from_socket.name, socketLinks.to_node.name, socketLinks.to_socket.name ])
        ...
        if not reload and linksRefresh == True:
            # remove sockets
            self.outputs.clear()
            ...
            # rebuild links
            for socketLinkElement in socketLinksArray:
                node_tree.links.new( node_tree.nodes[ socketLinkElement[ 0 ] ].outputs[ socketLinkElement[ 1 ] ], node_tree.nodes[ socketLinkElement[ 2 ] ].inputs[ socketLinkElement[ 3 ] ])

            # get and show numbers in sockets
            self.update_socket( bpy.context )
...
class SvTextOutNode(Node,SverchCustomTreeNode):
    ...
    line_modes = [("SL",   "Single line", "Single line",  1),
                  ("AL",   "Append line", "Append line",  2)]  

    line_mode = EnumProperty(items = line_modes, default='SL')
    ...
    def draw_buttons(self, context, layout):
        ...
        layout.prop(self,'line_mode',"Append")
